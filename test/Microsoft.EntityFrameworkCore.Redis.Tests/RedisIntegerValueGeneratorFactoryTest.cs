...
{
    public class RedisIntegerValueGeneratorFactoryTest
    {
        ...
        private static object CreateAndUseFactory(IProperty property)
            => new RedisIntegerValueGeneratorFactory().Create(property).Next(null);
        ...
        public void Throws_for_non_integer_property()
        {
            ...
            Assert.Equal(
                CoreStrings.InvalidValueGeneratorFactoryProperty(nameof(RedisIntegerValueGeneratorFactory), "BadCheese", "AnEntity"),
                Assert.Throws<ArgumentException>(() => new RedisIntegerValueGeneratorFactory().Create(property)).Message);
        }
